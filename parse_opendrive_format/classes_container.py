...
class Road(BaseClass):
    __slots__=["length", "is_in_junction", "predecessor_name", "predecessor_id", "successor_name", "successor_id", "left_driving_lane_ids", "right_driving_lane_ids"]
    def __init__(self, id, name, length, is_in_junction):
    ...
    def from_xodr(cls, xodr_road_element):
        ...
        road_class = cls(id = xodr_road_element.attrib["id"], name= xodr_road_element.attrib["name"], length= xodr_road_element.attrib["length"], is_in_junction= is_in_junction)
